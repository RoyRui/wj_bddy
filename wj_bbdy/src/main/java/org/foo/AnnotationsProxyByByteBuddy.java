	...
	public static void main(String[] args) {
		try {
			Class<?> runtimeType = new ByteBuddy().withAttribute(TypeAttributeAppender.ForSuperType.INSTANCE)
					...
					.method(named("toStringAgent")).intercept(FixedValue.value("Hello World!"))
					.make()
					.load(
			...
			if (runtimeType.getDeclaredMethod("getString").isAnnotationPresent(NBFoo.class)||runtimeType.getDeclaredMethod("toStringAgent").isAnnotationPresent(ToString.class)) {
